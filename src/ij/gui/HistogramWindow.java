...
public class HistogramWindow extends ImageWindow implements Measurements, ActionListener, 
	...
	void drawPlot(long maxCount, ImageProcessor ip) {
		...
		processEventImage(maxCount, ip, false);
	}
	...
	void drawLogPlot (long maxCount, ImageProcessor ip) {
		...
		processEventImage(max, ip, true);
		ip.setColor(Color.black);
	}

	void processEventImage(double max, ImageProcessor ip, boolean isDrawPlot) {
		if (histogram.length==256) {
			...
			for (int i=0; i < 256; i++) {
				int x =(int)(i*scale2);
				int y = isDrawPlot ? histogram[i]==0?0:(int)(HIST_HEIGHT*Math.log(histogram[i])/max): (int)(((double)HIST_HEIGHT*(double)histogram[i])/max);
			...
			for (int i = 0; i<HIST_WIDTH; i++) {
				...
				y = isDrawPlot ?  histogram[index]==0?0:(int)(HIST_HEIGHT*Math.log(histogram[index])/max): (int)(((double)HIST_HEIGHT*(double)histogram[index])/max);
				if (y>HIST_HEIGHT) y = HIST_HEIGHT;
			...
			for (int i=0; i<histogram.length; i++) {
				...
				if (value>0L) {
					int y = isDrawPlot ? (int)(HIST_HEIGHT*Math.log(value)/max): (int)(((double)HIST_HEIGHT*(double)value)/max);
					if (y>HIST_HEIGHT) y = HIST_HEIGHT;
